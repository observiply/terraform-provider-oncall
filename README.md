...
These resources are fully creatable and destroyable with `terraform apply` / `terraform destroy`:
...
Read-only data sources (not manageable by Terraform): `oncall_team`, `oncall_teams`,
`oncall_team_members`, `oncall_roles`, `oncall_resources`. Teams are
administrator-managed — look them up, don't create them.

See [`examples/complete/main.tf`](examples/complete/main.tf) for a realistic setup using all six.
...
see "Releasing" below for what's still required (provider-side acceptance
tests per `tfprovider-09-testing.md`, which are not done). Until then,
install it locally via `dev_overrides` (below).
...
.goreleaser.yml              release build: platform matrix, checksums, signing
...
the standard platform-matrix binaries, `SHA256SUMS`, and the versioned
`terraform-registry-manifest.json` copy the Terraform Registry expects — all
per `tfprovider-10-release.md` in oncall's `plan/`.

**Not yet done, and required before the first real tag:**

- **TODO: decide on artifact signing.** The GPG `signs:` block in
  `.goreleaser.yml` and the GPG-import step in `release.yml` are commented
  out pending a discussion — who owns the key, how it's rotated, whether we
  sign at all. The Terraform Registry needs a signed checksum file, so this
  must be resolved before publishing there.
...
Once the registry app is set up: register the repo with the
